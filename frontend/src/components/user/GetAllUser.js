...
function CustomTable({ list, fetchAllUsers }) {
...
const GetAllUser = (props) => {
  ...
  useEffect(() => {
    if (allCustomersInfo) {
      ...
      if (allCustomersInfo.getAllCustomers.aggregations)
  ...
  useEffect(() => {
    if (allCustomersError)
      console.log(allCustomersError)
  }, [allCustomersError]);

  useEffect(() => {
    fetchAllUsers({ status: "true" });
    // fetchAllCustomers({ query: "info" });
  }, []);
  ...
  return (
    <div className="card card-custom">
      <div className="card-body">
        <CustomTable list={list} total={total} fetchAllUsers={fetchAllUsers} />
      </div>
    </div>
  );
};
