...
function CustomTable({ list, total, fetchAllDesignations }) {
    ...
    <div>
      ...
      <Table
        ...
        pagination={{
          ...
          onChange: (page, limit) => {
            fetchAllDesignations({ page, limit })
...
const GetAllDesignation = (props) => {
  ...
  useEffect(() => {
    if (allDesignatonsData) {
      console.log(allDesignatonsData)
  ...
  useEffect(() => {
    fetchAllDesignations({ page: 1, limit: 10 });
  }, []);
  ...
  return (
    <div className="card column-design">
      <div className="card-body">
        <CustomTable list={list} fetchAllDesignations={fetchAllDesignations} />
      </div>
    </div>
  );
};
