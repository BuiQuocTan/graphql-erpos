...
import { GET_ALLTRANSACTION_QUERY } from "../../queries/transaction.query";
import { toast } from "react-toastify";

function CustomTable({ list, total, startdate, enddate, fetchAllTransactions }) {
		...
		<div>
			...
			<Table
				...
				pagination={{
					...
					onChange: (page, limit) => {
						fetchAllTransactions({ page, limit, startdate, enddate })
					},
				}}
...
const GetAllTransaction = (props) => {
	...
	const [getAllTransactions, { data: allTransactionInfo, error: allTransactionsError }] = useLazyQuery(GET_ALLTRANSACTION_QUERY);

	const fetchAllTransactions = useCallback(({ page, limit, startdate, enddate }) => {
		getAllTransactions({
			variables: {
	...
	useEffect(() => {
		if (allTransactionInfo) {
			console.log(allTransactionInfo.allTransactionsData)
			dispatch(loadAllTransaction(allTransactionInfo.allTransactionsData));
		}
	}, [allTransactionInfo]);
	...
	useEffect(() => {
		if (allTransactionsError) {
			...
			toast.error("Error occured while getting transactions");
		}
