						...
						<div className='top-container'>
							...
							<div className='bill-details'>
								<div className='flex justify-content-center'>
									<div>BILL NO: {(data && data.saleInvoiceProduct.length) ? data.saleInvoiceProduct[0].invoice_id : ""}</div>
								</div>
								<div className='flex justify-content-center'>
									<div>
...
const PosPrint = ({ data }) => {
	...
	const [getSetting, { data: settingData, error: getSettingError }] = useLazyQuery(GET_SETTING_QUERY);

	useEffect(() => {
		if (settingData) {
			console.log("setting data => ", settingData);
			setInvoiceData(settingData.getSetting);
		}
	}, [settingData]);
