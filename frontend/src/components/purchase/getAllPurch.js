...
function CustomTable({ list, total, status, setStatus, startdate, enddate, fetchAllPurchaseInvoice }) {
		...
		<div>
			...
			<Table
				...
				pagination={{
					...
					onChange: (page, limit) => {
						fetchAllPurchaseInvoice({ status, page, limit, startdate, enddate })
					},
				}}
...
const GetAllPurch = (props) => {
	...
	const [getAllPurchaseInvoice, { data: allPurchaseInvoiceInfo, error: allPurchaseInvoiceError }] = useLazyQuery(GET_ALLPURCHASE_QUERY);
	...
	useEffect(() => {
		if (allPurchaseInvoiceInfo) {
			console.log(allPurchaseInvoiceInfo);
			dispatch(loadAllPurchase(allPurchaseInvoiceInfo.getAllPurchaseInvoice))
		}
	}, [allPurchaseInvoiceInfo])

	useEffect(() => {
		if (allPurchaseInvoiceError)
			console.log(allPurchaseInvoiceError)
	}, [allPurchaseInvoiceError]);

	const fetchAllPurchaseInvoice = useCallback((props) => {
		getAllPurchaseInvoice({
			variables: props
		})
	});
			...
			<div className='card card-custom mt-1 '>
				<div className='card-body'>
					...
					<CustomTable
						...
						status={status}
						setStatus={setStatus}
						fetchAllPurchaseInvoice={fetchAllPurchaseInvoice}
					/>
				</div>
			</div>
		</>
