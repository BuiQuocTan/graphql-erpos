...
import { GET_ONESALEINVOICE_QUERY } from "../../queries/saleInvoice.query";
import { CREATE_RETURNSALEINVOICE_MUTATION } from "../../mutations/returnSaleInvoice.mutation";
...
const AddReturnSale = () => {
  ...
  const [getOneSaleInvoice, { data: oneSaleInvoiceData, error: oneSaleInvoiceError }] = useLazyQuery(GET_ONESALEINVOICE_QUERY);
  const [createReturnSaleInvoice, { data: createdReturnSaleInvoice, error: createReturnSaleInvoiceError }] = useMutation(CREATE_RETURNSALEINVOICE_MUTATION);

  useEffect(() => {
    if (oneSaleInvoiceData) {
      console.log("one sale invoice data =>", oneSaleInvoiceData);
      dispatch(loadSingleSale({ data: oneSaleInvoiceData.getSingleSaleInvoice }));
    }
  }, [oneSaleInvoiceData]);

  useEffect(() => {
    if (oneSaleInvoiceError) {
      console.log("one sale invoice error => ", oneSaleInvoiceError);
      toast.error("Error occured while gettign a sale invoice");
    }
  }, [oneSaleInvoiceError]);

  useEffect(() => {
    if (createdReturnSaleInvoice) {
      console.log("created sale invoice => ", createdReturnSaleInvoice);
      toast.success("Return Sale Added Successfully");
      setLoading(false);
      navigate(-1);
    }
  }, [createdReturnSaleInvoice]);

  useEffect(() => {
    if (createReturnSaleInvoiceError) {
      console.log("create sale invoice error");
      toast.error("something went wrong");
      setLoading(false);
    }
  }, [createReturnSaleInvoiceError])
