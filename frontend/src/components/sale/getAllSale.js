...
function CustomTable({ list, total, startdate, enddate, count, user, fetchAllSaleInvoice }) {
    ...
    <>
      ...
      <Table
        ...
        pagination={{
          ...
          onChange: (page, limit) => {
            fetchAllSaleInvoice({ page, limit, startdate, enddate, user })
          },
        }}
...
const GetAllSale = (props) => {
  ...
  useEffect(() => {
    if (allSaleInvoiceError) {
      setLoading(false);
      console.log(allSaleInvoiceError)
    }
  }, [allSaleInvoiceError])
  ...
  const onSearchFinish = async (values) => {
    ...
    fetchAllSaleInvoice({
      ...
      startdate: startdate,
      enddate: enddate,
      user: values.user ? values.user : 0,
    })
      ...
      <div className='card card-custom mt-1'>
        <div className='card-body'>
          ...
          <CustomTable
            ...
            count={count}
            user={user}
            fetchAllSaleInvoice={fetchAllSaleInvoice}
          />
        </div>
      </div>
    </>
